...
- (void)setupButtonWithY:(CGFloat)lastY
    ...
    button.frame = CGRectMake(120, lastY, 200 , 44);
...
- (void)viewDidLoad {
    ...
    [self simpleDanmaku];
    //[self timelineDanmaku];

    [self setupButtonWithY:500 title:@"获取弹幕" action:@selector(getData)];

}


- (void)timelineDanmaku{
    // ==================== 视频播放区域 ====================
...
- (void)getData{
    GetDanmuWebViewController *vc = [GetDanmuWebViewController new];
    vc.searchText = @"凡人修仙传";
    [vc setGetDanmuDataBlock:^(NSString * _Nonnull json) {
...
- (void)simpleDanmaku{
    // 创建透明容器视图 (盖在视频播放器上方)
    UIView *danmakuContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width , 400)];
    danmakuContainer.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.8];
    danmakuContainer.userInteractionEnabled = NO; // 关键: 事件穿透
    [self.view addSubview:danmakuContainer];
    danmakuContainer.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;

    // 初始化弹幕管理器
    self.danmakuManager = [[DanmakuManager alloc]
                           initWithContainerView:danmakuContainer
                           frame:danmakuContainer.bounds];
    [self.danmakuManager  configureWithDuration:5.0f alpha:0.8f density:DanmakuDensityHigh fontSize:16.0f];
}
